...
function autopep8_current_file()
  ...
  if filetype == "python" then
    ...
    local command = "autopep8 " .. current_file .. " --in-place"
    vim.fn.system(command)
    vim.cmd(":e")
  end
end

-- Set up an autocommand to trigger the function when saving a Python file
vim.cmd([[
    augroup Autopep8OnSave
        autocmd!
        autocmd BufWritePost *.py lua autopep8_current_file()
    augroup END
]])

-- set ctrl + backspace to delete a line
-- vim.api.nvim_set_keymap('i', '<C-H>', '<C-o><C-c>bdwi', { noremap = true, silent = true })
  ...
  function hjkl()
    for _, key in ipairs({"<left>", "<right>", "<up>", "<down>"}) do
      ...
      vim.keymap.set("n", key, function()
        ok, id = pcall(vim.notify, "Get good bro!🤌🤌", vim.log.levels.WARN, {
          icon = "🤠",
          replace = id,
          keep = function()
          end,
